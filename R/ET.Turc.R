...
ET.Turc <- function(data, constants, ts = "daily", solar = "sunshine hours",
    humid = F, message = "yes", save.csv = "yes", ...) 
{
    # class(data) <- funname
    ...
    if (humid == TRUE & (is.null(data$RHmax) | is.null(data$RHmin))) {
        # for adjustment for non-humid conditions
    ...
    if (humid == TRUE) {
        ...
        ET_Turc.Daily[RHmean < 50] <- 0.013 * (23.88 * R_s +
            50) * Ta[RHmean < 50]/(Ta[RHmean < 50] + 15) * (1 +
            (50 - RHmean[RHmean < 50])/70)  # Turc reference crop evapotranspiration adjusted for non-humid conditions (RH < 50) by Alexandris et al., (S9.11)
    }
    ...
    results <- list(ET.Daily = ET.Daily, ET.Monthly = ET.Monthly,
        ...
        ET_type = ET_type, message1 = message1, message4 = message4)
    ...
    if (save.csv == "yes") {
        ...
        for (i in 1:length(results)) {
            ...
            write.table(as.character(namer), file = "ET_Turc.csv",
                dec = ".", quote = FALSE, col.names = FALSE,
                row.names = F, append = TRUE, sep = ",")
            write.table(data.frame(get(namer, results)), file = "ET_Turc.csv",
