ET.Penman <- function(data, constants, ts = "daily", solar = "sunshine hours",
    ...
    if (wind == "no") {
        ...
        Surface <- paste("water, albedo =", alpha, "; roughness height =",
            z0, "m")
    } else {
        if (alpha != 0.08) {
    ...
    if (save.csv == "yes") {
        ...
        for (i in 1:length(results)) {
            ...
            write.table(as.character(namer), file = "ET_Penman.csv",
                dec = ".", quote = FALSE, col.names = FALSE,
                row.names = F, append = TRUE, sep = ",")
            write.table(data.frame(get(namer, results)), file = "ET_Penman.csv",
