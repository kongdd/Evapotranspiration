...
ReadInputs <- function(varnames, climatedata, constants, stopmissing,
    timestep = "daily", interp_missing_days = FALSE, interp_missing_entries = FALSE,
    interp_abnormal = FALSE, missing_method = NULL, abnormal_method = NULL) 
{
        ...
        if ("Tmax" %in% (colnames(climatedata))) {
            ...
            if (length(which(as.vector(Tmax.temp) > 100 | as.vector(Tmax.temp) <
                ...
                message(paste("Number of day increments when Tmax has errors (Tmax > 100 deg or < -50 deg): ",
                  length(which(as.vector(Tmax.temp) > 100 | as.vector(Tmax.temp) <
                    (-50)))))
    ...
    if ("RHmax" %in% varnames) {
        ...
        if ("RHmax" %in% (colnames(climatedata))) {
            RHmax.temp <- zoo(as.vector(climatedata$RHmax), dateagg)
    ...
    if ("RHmin" %in% varnames) {
        ...
        if ("RHmin" %in% (colnames(climatedata))) {
            RHmin.temp <- zoo(as.vector(climatedata$RHmin), dateagg)
    ...
    if (!"RHmax" %in% varnames & !"RHmin" %in% varnames) {
        if ("RH" %in% varnames) {
            ...
            if ("RH" %in% (colnames(climatedata))) {
                ...
                if (length(which(as.vector(RH.temp) > 100 | as.vector(RH.temp) <
                  0)) > 0) {
                  message(paste("Number of data entries where RH has errors (RH > 100% or < 0%): ",
                    length(which(as.vector(RH.temp) > 100 | as.vector(RH.temp) <
                      0))))
                  if (interp_abnormal == T) {
            ...
            if ("Precip" %in% (colnames(climatedata))) {
                ...
                if ("TRUE" %in% (is.na(climatedata$Precip))) {
                  ...
                  if (sum(is.na(climatedata$Precip)) >= stopmissing[2]/100 *
                    nrow(climatedata)) {
                    stop("missing data of Precip exceeds ", stopmissing[2],
                      "%, please use high quality data for calculation")
                  } else {
                    if (max(df)/nrow(climatedata) >= stopmissing[3]/100) {
    ...
    if ("vs" %in% varnames) {
        if (timestep == "daily") {
            if ("vs" %in% (colnames(climatedata))) {
                ...
                if (length(which(as.vector(vs.temp) < 0)) > 0) {
    ...
    if ("va" %in% varnames) {
        if (timestep == "daily") {
            if ("va" %in% (colnames(climatedata))) {
                ...
                if (length(which(as.vector(va.temp) < 0)) > 0) {
    ...
    data <- list(Date.daily = Date.daily, Date.monthly = Date.monthly,
        ...
        Tmin = Tmin, RHmax = RHmax, RHmin = RHmin, u2 = u2, uz = uz,
        Rs = Rs, n = n, Cd = Cd, Precip = Precip, P.monthly = P.monthly,
        Epan = Epan, va = va, vs = vs)
    invisible(data)
}

####################################
ReadInput_InterpAbnormal <- function(varname, upperdata = NULL,
    var, abnormal_method) {
    assign(varname, var)
    ...
    if (grepl("Tmax", varname) | grepl("Tdew", varname) | varname ==
        ...
        varname) | grepl("P", varname) | grepl("Epan", varname)) {
        limfun <- function(varname) {
            var <- get(varname)
        ...
        limfun <- function(varname) {
            var <- get(varname)
            test <- as.vector(var - upperdata) > 0 | as.vector(var) <
                (-20)
            return(test)
        }
    ...
    if (is.null(abnormal_method) | abnormal_method == "monthly average") {
        ...
        for (m in 0:11) {
            tempdata[which(as.POSIXlt(time(tempdata))$mon ==
                m & as.vector(limfun(varname)) == T)] = mean(tempdata[which(as.POSIXlt(time(tempdata))$mon ==
        ...
        for (s in 1:4) {
            m = c(smonth[s, ])
            tempdata[which(any(as.POSIXlt(time(tempdata))$mon ==
                m) & as.vector(limfun(varname)) == T)] = mean(tempdata[which(as.POSIXlt(time(tempdata))$mon ==
                m & as.vector(limfun(varname)) == F)])
        }
    } else if (abnormal_method == "DoY average") {
        for (j in 1:366) {
            tempdata[which(as.numeric(strftime(time(tempdata),
                format = "%j")) == j & as.vector(limfun(varname)) ==
                T)] = mean(tempdata[which(as.numeric(strftime(time(tempdata),
                format = "%j")) == j & as.vector(limfun(varname)) ==
                F)])
        }
...
ReadInput_InterpMissing <- function(varname, var, missing_method) {
    ...
    if (is.null(missing_method) | missing_method == "monthly average") {
        ...
        for (m in 0:11) {
            tempdata[which(as.POSIXlt(time(tempdata))$mon ==
                m & is.na(tempdata))] = mean(tempdata[which(as.POSIXlt(time(tempdata))$mon ==
                m & !is.na(tempdata))])
        }
        ...
        for (j in 1:366) {
            tempdata[which(as.numeric(strftime(time(tempdata),
                format = "%j")) == j & is.na(tempdata))] = mean(tempdata[which(as.numeric(strftime(time(tempdata),
                format = "%j")) == j & !is.na(tempdata))])
        }
        ...
        if (length(tempdata) %in% misi) {
            tempdata[length(tempdata)] = tempdata[length(tempdata) -
                1]
        }
        for (i in misi[misi != 1 & misi != length(tempdata)]) {
            if (!(i - 1) %in% misi) {
                # means i is the start of missing value means i is the finish
                # of missing value i.e. only one missing
                if (!(i + 1) %in% misi) {
                  tempdata[i] = mean(c(tempdata[i + 1], tempdata[i -
                    1]))
                  misi <- setdiff(misi, i)
                } else {
                  # means i is not the finish of missing value i.e. more than
                  # one missing stop('More than one entry missing, please
                  # choose another interpolation method')
                  tempdata = NULL
                  misi = 0
                  break
                  # fi <- c(misi[misi>i & !(misi+1)%in%misi])[1] nmi <- fi-i+1
                  # tempdata[i:fi] <-
                  # as.vector(tempdata[i-1])+(as.vector(tempdata[fi+1])-as.vector(tempdata[i-1]))/(nmi+1)*(1:nmi)
        ...
        # } else if (timestep == 'subdaily') { misi <-
        # which(is.na(tempdata[1:length(Date.subdaily)])) if
        # (1%in%misi) { tempdata[1] = tempdata[2] } if
        # (length(Date.subdaily)%in%misi) {
        # tempdata[length(Date.subdaily)] =
        # tempdata[length(Date.subdaily)-1] } for (i in
        # misi[misi!=1&misi!=length(Date.subdaily)]) { if
        # (!(i-1)%in%misi) { # means i is the start of missing value
        # if (!(i+1)%in%misi) { # means i is the finish of missing
        # value i.e. only one missing tempdata[i] =
        # mean(c(tempdata[i+1],tempdata[i-1])) misi <-
        # setdiff(misi,i) } else { # means i is not the finish of
        # missing value i.e. more than one missing fi <-
        # c(misi[misi>i & !(misi+1)%in%misi])[1] nmi <- fi-i+1
        # tempdata[i:fi] <-
        # as.vector(tempdata[i-1])+(as.vector(tempdata[fi+1])-as.vector(tempdata[i-1]))/(nmi+1)*(1:nmi)
        # misi <- setdiff(misi,i:fi) } } if (length(misi) == 0) break
    }
...
ReadOBSEvaporation <- function(E_OBS, data) {
    ...
    Date.OBS <- strptime(paste(E_OBS$Day, "/", E_OBS$Month, "/",
        E_OBS$Year, sep = ""), "%d/%m/%Y")
    if (E_OBS$Day[1] == E_OBS$Day[2]) {
        ...
        E_obs.Daily <- NULL
        E_obs.Monthly <- E_obs
        E_obs.Annual <- aggregate(E_obs.Monthly, floor(as.numeric(as.yearmon(Date.OBS,
            "%y"))), FUN = sum)
        ...
        for (mon in min(as.POSIXlt(Date.OBS)$mon):max(as.POSIXlt(Date.OBS)$mon)) {
            ...
            E_obs.MonthlyAve[i] <- mean(E_obs.MonthlyAve.temp[as.POSIXlt(Date.OBS)$mon ==
                mon])
        }
        for (year in min(as.POSIXlt(Date.OBS)$year):max(as.POSIXlt(Date.OBS)$year)) {
            i = year - min(as.POSIXlt(Date.OBS)$year) + 1
            E_obs.AnnualAve[i] <- mean(E_obs.MonthlyAve.temp[as.POSIXlt(Date.OBS)$year ==
                year])
        }
    } else {
        ...
        # Aggregation
        E_obs.Daily <- E_obs
        E_obs.Monthly <- aggregate(E_obs, as.yearmon(Date.OBS,
            "%m/%y"), FUN = sum)
        E_obs.Annual <- aggregate(E_obs.Daily, floor(as.numeric(as.yearmon(Date.OBS,
            "%y"))), FUN = sum)
        ...
        for (mon in min(as.POSIXlt(Date.OBS)$mon):max(as.POSIXlt(Date.OBS)$mon)) {
            i = mon - min(as.POSIXlt(Date.OBS)$mon) + 1
            E_obs.MonthlyAve[i] <- mean(E_obs.Daily[as.POSIXlt(Date.OBS)$mon ==
                mon])
        }
        for (year in min(as.POSIXlt(Date.OBS)$year):max(as.POSIXlt(Date.OBS)$year)) {
            i = year - min(as.POSIXlt(Date.OBS)$year) + 1
            E_obs.AnnualAve[i] <- mean(E_obs.Daily[as.POSIXlt(Date.OBS)$year ==
                year])
        }
    }

    OBS <- list(Date.OBS = Date.OBS, E_obs.Daily = E_obs.Daily,
        E_obs.Monthly = E_obs.Monthly, E_obs.Annual = E_obs.Annual,
        E_obs.MonthlyAve = E_obs.MonthlyAve, E_obs.AnnualAve = E_obs.AnnualAve)
    return(OBS)
}
