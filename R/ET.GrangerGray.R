ET.GrangerGray <- function(data, constants, ts = "daily", solar = "sunshine hours",
    ...
    ...)
{
    ...
    # Calculations from data and constants for Granger and Gray
    ...
    G_g <- 1/(0.793 + 0.2 * exp(4.902 * D_p)) + 0.006 * D_p  # dimensionless evaporation parameter (S8.5)
    ET_GG_Act.Daily <- delta * G_g/(delta * G_g + gamma) * (R_ng -
        constants$G)/constants$lambda + gamma * G_g/(delta *
        G_g + gamma) * Ea  # Granger and Gray actual areal evapotranspiration (mm.day^-1) Granger and Gray (1989) (S8.4)
    ...
    if (save.csv == "yes") {
        ...
        for (i in 1:length(results)) {
            ...
            write.table(as.character(namer), file = "ET_GrangerGray.csv",
                dec = ".", quote = FALSE, col.names = FALSE,
                row.names = F, append = TRUE, sep = ",")
            write.table(data.frame(get(namer, results)), file = "ET_GrangerGray.csv",
